...
void inserir(Aluno *DB, int *count, int max) {    
    if (*count < max) { 
        ler(&DB[*count]); 
        (*count)++;
...
int menu(Aluno *DB, int *count, int max) {
    int opcao;
    ...
    do {
        ...
        switch (opcao) {
            case 1: inserir(DB, count, max); break;
...
int main(int argc, char **argv) {
    ...
    puts("Maximo de alunos: ");
    scanf("%d", &tam);
    if (tam) {
        DB = (Aluno *)malloc(tam * sizeof(Aluno));
        menu(DB, &count, tam);
    }
